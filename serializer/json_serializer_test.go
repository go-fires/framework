...
func TestJsonSerializer_String(t *testing.T) {
	serialized, err := JsonSerializer.Serialize("foo")
	assert.Nil(t, err)
	assert.Equal(t, "\"foo\"", string(serialized))

	var unserialize string
	assert.Nil(t, JsonSerializer.Unserialize(serialized, &unserialize))
	assert.Equal(t, "foo", unserialize)
}

func TestJsonSerializer_Int(t *testing.T) {
	serialized, err := JsonSerializer.Serialize(1)
	assert.Nil(t, err)
...
func TestJsonSerializer_Struct(t *testing.T) {
	...
	serialized, err := JsonSerializer.Serialize(user)
	assert.Nil(t, err)
